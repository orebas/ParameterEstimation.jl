...
function check_identifiability(ode::ModelingToolkit.ODESystem;
	...
	params_to_assess_ = ParameterEstimation.get_parameters(ode_prep)
	nemo2mtk = Dict(gens_ .=> input_syms)

	#res2 = SIAN.identifiability_ode(ode_prep, params_to_assess_; p = 0.99,
...
function identifiability_ode(ode, params_to_assess; p = 0.99, p_mod = 0, infolevel = 0,
	...
	eqs, Q, x_eqs, y_eqs, x_vars, y_vars, u_vars, mu, all_indets, gens_Rjet = ParameterEstimation.get_equations(ode)
	...
	# (b,c) -------------
	X, X_eq = ParameterEstimation.get_x_eq(x_eqs, y_eqs, n, m, s, u, gens_Rjet)

	# (d,e) -----------
	Y, Y_eq = ParameterEstimation.get_y_eq(x_eqs, y_eqs, n, m, s, u, gens_Rjet)

	# 2.Truncate
	@info "Truncating"

	# (a) -----------------------
	d0 = BigInt(maximum(vcat([Nemo.total_degree(ParameterEstimation.unpack_fraction(Q * eq[2])[1])
	...
	beta = [0 for i in 1:m]
	Et = Array{Nemo.fmpq_mpoly}(undef, 0)
	x_theta_vars = all_params
	prolongation_possible = [1 for i in 1:m]

	# (f) ------------------
	all_x_theta_vars_subs = ParameterEstimation.insert_zeros_to_vals(all_subs[1], all_subs[2])
	eqs_i_old = Array{Nemo.fmpq_mpoly}(undef, 0)
	evl_old = Array{Nemo.fmpq_mpoly}(undef, 0)
	while sum(prolongation_possible) > 0
		for i in 1:m
			if prolongation_possible[i] == 1
				...
				JacX = ParameterEstimation.jacobi_matrix(evl_old, x_theta_vars, all_x_theta_vars_subs)
				eqs_i_old = eqs_i
				if LinearAlgebra.rank(JacX) == length(eqs_i)
					...
					while length(polys_to_process) != 0
						new_to_process = Array{Nemo.fmpq_mpoly}(undef, 0)
						vrs = Set{Nemo.fmpq_mpoly}()
						for poly in polys_to_process
							vrs = union(vrs,
								[v
								 for v in Nemo.vars(poly) if v in x_variables])
						end
						vars_to_add = Set{Nemo.fmpq_mpoly}(v
														   for v in vrs
														   if !(v in x_theta_vars))
						for v in vars_to_add
							x_theta_vars = vcat(x_theta_vars, v)
							ord_var = ParameterEstimation.get_order_var2(v, all_indets, n + m + u, s)
	...
	for i in 1:m
		for j in (beta[i]+1):length(Y[i])
			to_add = true
			for v in ParameterEstimation.get_vars(Y[i][j], x_vars, all_indets, n + m + u, s)
				if !(v in x_theta_vars)
					to_add = false
				end
			end
	...
	theta_l = Array{Nemo.fmpq_mpoly}(undef, 0)
	params_to_assess_ = [ParameterEstimation.add_to_var(param, Rjet, 0) for param in params_to_assess]
	...
	for param_0 in params_to_assess_
		...
		JacX = ParameterEstimation.jacobi_matrix(Et_subs, other_params, all_x_theta_vars_subs)
	...
	if length(theta_l) == 0
		...
		return Dict("identifiability" => Dict("locally_identifiable" => [],
			...
			"non_identifiable" => Set(ParameterEstimation.get_order_var(th,
				non_jet_ring)[1]
									  for th in params_to_assess)))
	else
		@info "Locally identifiable parameters: [$(join([ParameterEstimation.get_order_var(th, non_jet_ring)[1] for th in theta_l], ", "))]"
		@info "Not identifiable parameters:     [$(join([ParameterEstimation.get_order_var(th, non_jet_ring)[1] for th in setdiff(params_to_assess_, theta_l)], ", "))]"

		non_identifiable_parameters = Set(ParameterEstimation.get_order_var(th, non_jet_ring)[1]
		...
		if weighted_ordering
			weights = ParameterEstimation.get_weights(ode, non_identifiable_parameters)
		end
		# 3. Randomize.
		@info "Randomizing"
		# (a) ------------
		...
		D2 = floor(BigInt,
			...
			(1 - p))
		# (b, c) ---------
		sample = ParameterEstimation.sample_point(D2, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq,
			Q)
		y_hat = sample[1]
		u_hat = sample[2]
		theta_hat = sample[3]
		...
		vrs_sorted = vcat(sort([e for e in Et_x_vars],
				lt = (x, y) -> ParameterEstimation.compare_diff_var(x, y, all_indets,
					n + m + u, s)), z_aux,
		...
		if weighted_ordering
			for i in eachindex(Et_hat)
				for _var in Set(Nemo.vars(Et_hat[i]))
					_var_non_jet, _var_order = ParameterEstimation.get_order_var(_var, non_jet_ring)
					Et_hat[i] = make_substitution(Et_hat[i], _var,
			...
			for _var in Set(Nemo.vars(Q_hat))
				_var_non_jet, _var_order = ParameterEstimation.get_order_var(_var, non_jet_ring)
				Q_hat = make_substitution(Q_hat, _var,
					_var^get(weights, _var_non_jet, 1),
					parent(_var)(1))
			end
		end
		...
		if p_mod > 0
			Et_hat = [ParameterEstimation._reduce_poly_mod_p(e, p_mod) for e in Et_hat]
			z_aux = ParameterEstimation._reduce_poly_mod_p(z_aux, p_mod)
			Q_hat = ParameterEstimation._reduce_poly_mod_p(Q_hat, p_mod)
		...
		Et_hat = [ParameterEstimation.parent_ring_change(e, Rjet_new) for e in Et_hat]
		gb = groebner(vcat(Et_hat, ParameterEstimation.parent_ring_change(z_aux * Q_hat, Rjet_new) - 1))

		theta_g = Array{Any}(undef, 0)
		@info "Remainder computation"

		if p_mod > 0
			theta_l_new = [ParameterEstimation.parent_ring_change(ParameterEstimation._reduce_poly_mod_p(th, p_mod),
				Rjet_new) for th in theta_l]

			for i in eachindex(theta_l)
				_var_non_jet, _var_order = ParameterEstimation.get_order_var(theta_l_new[i], non_jet_ring)
				if Groebner.normalform(gb, theta_l_new[i]^get(weights, _var_non_jet, 1)) ==
				   ParameterEstimation.parent_ring_change(ParameterEstimation._reduce_poly_mod_p(Rjet(theta_hat[2][findfirst(isequal(theta_l[i]),
							theta_hat[1])]),
						p_mod), Rjet_new)
			...
			theta_l_new = [ParameterEstimation.parent_ring_change(th, Rjet_new) for th in theta_l]

			for i in eachindex(theta_l)
				_var_non_jet, _var_order = ParameterEstimation.get_order_var(theta_l_new[i], non_jet_ring)
				if Groebner.normalform(gb, theta_l_new[i]^get(weights, _var_non_jet, 1)) ==
				   ParameterEstimation.parent_ring_change(Rjet(theta_hat[2][findfirst(isequal(theta_l[i]),
						theta_hat[1])]),
					Rjet_new)
					theta_g = vcat(theta_g, theta_l_new[i])
				end
		...
		identifiability_result = Dict("globally" => Set(ParameterEstimation.get_order_var(th,
				non_jet_ring)[1]
							   for th in theta_g),
			"locally_not_globally" => Set(ParameterEstimation.get_order_var(th,
				non_jet_ring)[1]
										  for th in setdiff(theta_l_new,
				theta_g)),
			"nonidentifiable" => Set(ParameterEstimation.get_order_var(th,
				non_jet_ring)[1]
		...
		for each in Y_eq
			name, order = ParameterEstimation.get_order_var(each[1], non_jet_ring)
			y_derivative_dict[each[1]] = order
		end
		u_derivative_dict = Dict()
		for each in u_variables
			name, order = ParameterEstimation.get_order_var(each, non_jet_ring)
