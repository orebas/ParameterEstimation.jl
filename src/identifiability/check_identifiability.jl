...
[2] -  software for structural identifiability analysis of ODE models (Bioinformatics, Volume 35, Issue 16, Pages 2873--2874, 2019), https://academic.oup.com/bioinformatics/article/35/16/2873/5096881

[3] - https://github.com/alexeyovchinnikov/Julia
"""
 function check_identifiability(ode::ModelingToolkit.ODESystem;
    ...
    params_to_assess_ = get_parameters(ode_prep)
...
Base function for identifiability assessment implmented in jl. Detailed documentation is available in
`check_identifiability` function as well as in https://github.com/alexeyovchinnikov/Julia.
"""
function identifiability_ode(ode, params_to_assess; p = 0.99, p_mod = 0, infolevel = 0,
    ...
    # (b,c) -------------
    X, X_eq = get_x_eq(x_eqs, y_eqs, n, m, s, u, gens_Rjet)

    # (d,e) -----------
    Y, Y_eq = get_y_eq(x_eqs, y_eqs, n, m, s, u, gens_Rjet)

    # 2.Truncate
    @info "Truncating"

    # (a) -----------------------
    d0 = BigInt(maximum(vcat([Nemo.total_degree(unpack_fraction(Q * eq[2])[1])
    ...
    params_to_assess_ = [add_to_var(param, Rjet, 0) for param in params_to_assess]
    ...
    if length(theta_l) == 0
        ...
        return Dict("identifiability" => Dict("locally_identifiable" => [],
                                              ...
                                              "non_identifiable" => Set(get_order_var(th,
                                                                                           non_jet_ring)[1]
                                                                        for th in params_to_assess)))
    else
        @info "Locally identifiable parameters: [$(join([get_order_var(th, non_jet_ring)[1] for th in theta_l], ", "))]"
        @info "Not identifiable parameters:     [$(join([get_order_var(th, non_jet_ring)[1] for th in setdiff(params_to_assess_, theta_l)], ", "))]"

        non_identifiable_parameters = Set(get_order_var(th, non_jet_ring)[1]
        ...
        if weighted_ordering
            weights = get_weights(ode, non_identifiable_parameters)
        end
        # 3. Randomize.
        @info "Randomizing"
        # (a) ------------
        ...
        D2 = floor(BigInt,
                   ...
                   (1 - p))
        # (b, c) ---------
        sample = sample_point(D2, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq,
                                   Q)
        y_hat = sample[1]
        u_hat = sample[2]
        theta_hat = sample[3]
        ...
        vrs_sorted = vcat(sort([e for e in Et_x_vars],
                               lt = (x, y) -> compare_diff_var(x, y, all_indets,
                                                                    n + m + u, s)), z_aux,
        ...
        if weighted_ordering
            for i in eachindex(Et_hat)
                for _var in Set(Nemo.vars(Et_hat[i]))
                    _var_non_jet, _var_order = get_order_var(_var, non_jet_ring)
        ...
        if p_mod > 0
            theta_l_new = [parent_ring_change(_reduce_poly_mod_p(th, p_mod),
                                                   Rjet_new) for th in theta_l]

            for i in eachindex(theta_l)
                _var_non_jet, _var_order = get_order_var(theta_l_new[i], non_jet_ring)
                if Groebner.normalform(gb, theta_l_new[i]^get(weights, _var_non_jet, 1)) ==
                   parent_ring_change(_reduce_poly_mod_p(Rjet(theta_hat[2][findfirst(isequal(theta_l[i]),
                                                                                               theta_hat[1])]),
                                                                   p_mod), Rjet_new)
            ...
            for i in eachindex(theta_l)
                _var_non_jet, _var_order = get_order_var(theta_l_new[i], non_jet_ring)
                if Groebner.normalform(gb, theta_l_new[i]^get(weights, _var_non_jet, 1)) ==
                   parent_ring_change(Rjet(theta_hat[2][findfirst(isequal(theta_l[i]),
                                                                       theta_hat[1])]),
                                           Rjet_new)
                    theta_g = vcat(theta_g, theta_l_new[i])
                end
        ...
        identifiability_result = Dict("globally" => Set(get_order_var(th,
                                                                           non_jet_ring)[1]
                                                        for th in theta_g),
                                      "locally_not_globally" => Set(get_order_var(th,
                                                                                       non_jet_ring)[1]
                                                                    for th in setdiff(theta_l_new,
                                                                                      theta_g)),
                                      "nonidentifiable" => Set(get_order_var(th,
                                                                                  non_jet_ring)[1]
        ...
        for each in Y_eq
            name, order = get_order_var(each[1], non_jet_ring)
            y_derivative_dict[each[1]] = order
        end
        u_derivative_dict = Dict()
        for each in u_variables
            name, order = get_order_var(each, non_jet_ring)
