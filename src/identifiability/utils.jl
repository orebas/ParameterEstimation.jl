...
function count_solutions(identifiability_result)
	...
	for param in globally_id
		v = get_order_var(param, non_jet_ring)[1]
	...
	basis_sing = [parent_ring_change(identifiability_result["basis"][i], OR)
				  for i in 1:length(identifiability_result["basis"])]
	locally_not_globally_id_sing = [parent_ring_change(locally_not_globally_id[i],
		OR)
	...
	for (idx, param) in enumerate(locally_not_globally_id_sing)
		...
		v = get_order_var(locally_not_globally_id[idx], non_jet_ring)[1]
